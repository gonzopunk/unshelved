-- Add Genre axis for existing users
INSERT INTO public.tag_axes (user_id, key, label, kind, scale_min, scale_max, values, open, position, built_in)
SELECT
  p.id, 'genre', 'Genre', 'multi', NULL, NULL,
  ARRAY['Fiction','Nonfiction','Literary Fiction','Science Fiction',
        'Fantasy','Mystery','Thriller','Historical Fiction',
        'Biography','Short Stories','Poetry','Essays'],
  true, 10, true
FROM public.profiles p
WHERE NOT EXISTS (
  SELECT 1 FROM public.tag_axes WHERE user_id = p.id AND key = 'genre'
);

-- Add Weight axis for existing users (values column is NOT NULL, use empty array)
INSERT INTO public.tag_axes (user_id, key, label, kind, scale_min, scale_max, values, open, position, built_in)
SELECT
  p.id, 'weight', 'Weight', 'scale', 1, 5,
  '{}'::text[], false, 11, true
FROM public.profiles p
WHERE NOT EXISTS (
  SELECT 1 FROM public.tag_axes WHERE user_id = p.id AND key = 'weight'
);

-- Update seed_tag_axes to include Genre and Weight for new users
...
BEGIN
  INSERT INTO public.tag_axes (user_id, key, label, kind, scale_min, scale_max, values, open, position, built_in) VALUES
    ...
    (_user_id, 'genre', 'Genre', 'multi', null, null,
      array['Fiction','Nonfiction','Literary Fiction','Science Fiction','Fantasy','Mystery','Thriller','Historical Fiction','Biography','Short Stories','Poetry','Essays'], true, 10, true),
    (_user_id, 'weight', 'Weight', 'scale', 1, 5, '{}', false, 11, true)
