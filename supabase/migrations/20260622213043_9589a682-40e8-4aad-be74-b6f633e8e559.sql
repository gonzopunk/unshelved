ALTER TABLE public.connections
  ADD COLUMN strength integer NOT NULL DEFAULT 3
  CONSTRAINT connections_strength_check CHECK (strength >= 1 AND strength <= 5);
...
begin
  ...
  insert into public.connections (user_id, source_kind, source_id, target_kind, target_id, why, tags, is_sample, strength) values
    (_user_id, 'book', b1, 'reference_book', r2, 'Both ask whether trees and woods deserve our moral attention.', array['nature','ethics'], true, 4),
    (_user_id, 'book', b2, 'book', b5, 'Solitary narrators piecing together a world they only half understand.', array['voice','solitude'], true, 3),
    (_user_id, 'book', b2, 'reference_book', r3, 'Vast halls and statues — Piranesi feels like a grown-up Wood Between the Worlds.', array['architecture'], true, 2),
    (_user_id, 'book', b3, 'reference_book', r5, 'Weir doing Weir: one man, impossible odds, lots of math.', array['author'], true, 2),
    (_user_id, 'book', b4, 'reference_book', r4, 'Multigenerational family epics about land, identity, and inheritance.', array['family','epic'], true, 4),
    (_user_id, 'book', b5, 'reference_book', r6, 'A made being trying to understand the humans who made her.', array['ai','creation'], true, 5),
    (_user_id, 'book', b6, 'book', b5, 'Friendship and creation as the things we keep building each other into.', array['friendship'], true, 4),
    (_user_id, 'book', b7, 'reference_book', r4, 'Confined men finding the whole human comedy in a single setting.', array['confinement'], true, 3),
    (_user_id, 'highlight', h1, 'reference_book', r2, 'Powers'' line about story echoes Thoreau''s belief that we live by the tales we tell of the woods.', array['story','nature'], true, 4),
    (_user_id, 'book', b1, 'book', b4, 'Both novels braid many lives into one slow-growing whole.', array['structure'], true, 3),
    (_user_id, 'book', b11, 'book', b3, 'Quiet melancholic SF where one figure carries the weight of a whole timeline.', array['sf','time'], true, 3),
    (_user_id, 'book', b11, 'reference_book', r7, 'Nested timelines that rhyme rather than repeat.', array['structure','time'], true, 5),
    (_user_id, 'book', b13, 'book', b5, 'Outsider narrators decoding a culture not built for them.', array['other','voice'], true, 4),
    (_user_id, 'book', b12, 'book', b10, 'Maggie Nelson in two registers — fragment vs. fugue.', array['author','essay'], true, 5),
    (_user_id, 'book', b9, 'reference_book', r8, 'Two halves of one autobiography: youth, then grief.', array['memoir'], true, 5),
    (_user_id, 'book', b15, 'reference_book', r9, 'American capital as the unreliable narrator.', array['money','americana'], true, 4),
    (_user_id, 'book', b14, 'book', b8, 'Faith inherited and faith fought for.', array['family','belief'], true, 3),
    (_user_id, 'highlight', h8, 'book', b5, 'Both treat color and light as moral attention.', array['color','perception'], true, 4);
